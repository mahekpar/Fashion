...
def match(id):
    ...
    if uploaded_file is not None:
            ...
            with co2:
                st.write(f"### {df.iloc[0,9]}")
                # st.write('---')
                st.write(f"#### {'Rs. ' + str(df.iloc[0,10])}")
                st.write('Gender: ' + str(df.iloc[0,1]))
                st.write('Category: ' + str(df.iloc[0,3]))
                st.write('Article Type: ' + str(df.iloc[0,4]))
                st.write('Season: ' + str(df.iloc[0,6]))
                st.write('Occasion: ' + str(df.iloc[0,8]))
